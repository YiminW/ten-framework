...
class ElevenLabsTTS2:
    def __init__(self, config: ElevenLabsTTS2Config, ten_env: AsyncTenEnv, error_callback: Callable[[str, ModuleError], Awaitable[None]] = None) -> None:
        self.config = config
        self.ws = None
    ...
    async def start_connection(self):
        """启动 WebSocket 连接"""
        try:
            self.ten_env.log_info(f"Starting WebSocket connection to {self.uri}")
            ...
            self.ws_send_task = asyncio.create_task(self.text_to_speech_ws_streaming())
            ...
            self.ten_env.log_info("WebSocket connection established successfully")
    ...
    async def ws_recv_loop(self):
        """WebSocket 接收循环，处理音频数据"""
        while self.is_connected:
            try:
                if not self.ws or self.ws.state.name == 'CLOSED':
                    self.ten_env.log_warning("WebSocket is closed, attempting to reconnect...")
                ...
                if data.get("alignment"):
                    ...
                    if alignment.get("chars"):
                        ...
                        for char in chars:
                            text += char
                    self.ten_env.log_debug(f"Received alignment from WebSocket: {text}")
                if data.get("isFinal"):
                ...
                if isFinal:
                    self.ten_env.log_info("Received final message from WebSocket")
                    return
                if data.get("error"):
                    error_info = ModuleErrorVendorInfo(
                        vendor="elevenlabs",
                ...
                self.ten_env.log_error(f"Failed to parse WebSocket message: {e}")
                continue
    ...
    async def text_to_speech_ws_streaming(self):
        """WebSocket 发送循环，处理文本输入"""
        while self.is_connected:
            try:
                if not self.ws or self.ws.state.name == 'CLOSED':
                    self.ten_env.log_warning("WebSocket is closed in send loop, waiting for reconnection...")
                ...
                if t.text.strip() != "":
                    ...
                    self.ten_env.log_debug(f"Sent text to WebSocket: {t.text[:50]}...")
                ...
                self.ten_env.log_warning(f"WebSocket connection closed in send loop: {e}")
                await self._handle_reconnection()
            except websockets.exceptions.WebSocketException as e:
                self.ten_env.log_error(f"WebSocket error in send loop: {e}")
                await self._handle_reconnection()
            except asyncio.CancelledError:
                self.ten_env.log_info("text_to_speech_ws_streaming task cancelled")
                break
            except Exception as e:
                self.ten_env.log_error(f"Unexpected error in text_to_speech_ws_streaming: {e}")
                await self._handle_reconnection()

    async def _handle_reconnection(self):
        """处理重连逻辑"""
        if self.reconnect_attempts >= self.max_reconnect_attempts:
            self.ten_env.log_error(f"Max reconnection attempts ({self.max_reconnect_attempts}) reached")
            self.is_connected = False
        ...
        self.ten_env.log_info(f"Attempting to reconnect in {delay} seconds (attempt {self.reconnect_attempts}/{self.max_reconnect_attempts})")
    ...
    def is_connection_healthy(self) -> bool:
        ...
        return (self.is_connected and 
                self.ws and 
                self.ws.state.name != 'CLOSED' and
                self.ws_recv_task and 
                not self.ws_recv_task.done() and
                self.ws_send_task and 
                not self.ws_send_task.done())
