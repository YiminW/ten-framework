...
class ElevenLabsTTS2Extension(AsyncTTS2BaseExtension):
    def __init__(self, name: str) -> None:
        ...
        self.recorder_map: dict[str, PCMWriter] = {}  # store different request id pcmwriter
    ...
    async def on_init(self, ten_env: AsyncTenEnv) -> None:
        try:
            ...
            async def error_callback(request_id: str, error: ModuleError):
                # 如果没有提供 request_id，使用当前的 request_id
            ...
            self.read_audio_data_task = asyncio.create_task(self.read_audio_data())
        except Exception as e:
            ten_env.log_error(f"on_init failed: {traceback.format_exc()}")
            await self.send_tts_error(
                "",  # 在 on_init 时还没有 request_id
                ModuleError(
                    message=str(e),
    ...
    async def request_tts(self, t: TTSTextInput) -> None:
        ...
        try:
            ...
            if self.completed_request_ids and t.request_id in self.completed_request_ids:
    ...
    async def on_data(self, ten_env: AsyncTenEnv, data: Data) -> None:  
        ...
        if name == "tts_flush":
            ...
            ten_env.log_info(f"Sent tts_audio_end with INTERRUPTED reason for request_id: {flush_id}")
    ...
    async def read_audio_data(self):
        try:
            while True:
                ...
                # 检查特殊停止信号
                if text == "STOP_LOOP":
                    self.ten_env.log_info("Received STOP_LOOP signal, exiting read_audio_data")
                    break
                ...
                if self.current_request_id and self.current_request_id in self.flush_request_ids:
                    ...
                    if audio_data is not None:
                        if self.config.dump and self.current_request_id and self.current_request_id in self.recorder_map:
                            await self.recorder_map[self.current_request_id].write(audio_data)
                ...
                if isFinal and self.current_request_id:
                    await self.client.start_connection()
                    self.ten_env.log_info(f"Request ID {self.current_request_id} is final, send TTSAudioEnd event")
                    if self.request_start_ts is not None:
                        request_event_interval = int(
                            (
                                datetime.now() - self.request_start_ts
                    ...
                    self.ten_env.log_info(f"add completed request_id to: {self.current_request_id}")
        except asyncio.CancelledError:
            self.ten_env.log_info("read_audio_data task cancelled")
            raise
        except Exception as e:
            self.ten_env.log_error(f"Error in read_audio_data: {traceback.format_exc()}")