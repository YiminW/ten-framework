...
class ExtensionTesterMetricsWithMetadata(ExtensionTester):
    ...
    def on_start(self, ten_env_tester: TenEnvTester) -> None:
        ...
        ten_env_tester.log_info("Metrics with metadata test started, sending TTS request.")
...
def test_metrics_with_metadata(MockElevenLabsTTS2):
    ...
    assert tester.audio_start_data is not None, "Audio start data was not received"
    assert "request_id" in tester.audio_start_data, "Audio start missing request_id"
    ...
    assert "request_event_interval_ms" in tester.audio_end_data, "Audio end missing interval"
    assert "request_total_audio_duration_ms" in tester.audio_end_data, "Audio end missing duration"
