...
class ExtensionTesterErrorHandling(ExtensionTester):
    ...
    def on_start(self, ten_env_tester: TenEnvTester) -> None:
        ...
        ten_env_tester.log_info("Error handling test started, sending TTS request.")
    ...
    def on_data(self, ten_env: TenEnvTester, data) -> None:
        ...
        # 如果收到任何数据，说明 on_init 成功了，我们可以继续
...
def test_vendor_exception_handling(MockElevenLabsTTS2):
    ...
    mock_client_instance.start_connection.side_effect = ModuleVendorException(error_info)
...
def test_general_exception_handling(MockElevenLabsTTS2):
    ...
    mock_client_instance.start_connection.side_effect = Exception("Test general error")
    MockElevenLabsTTS2.return_value = mock_client_instance
    ...
    assert vendor_info["vendor"] == "", "Vendor should be empty for general exceptions"
    assert vendor_info["code"] == "", "Code should be empty for general exceptions"
    assert vendor_info["message"] == "", "Message should be empty for general exceptions"
...
class ExtensionTesterFlushError(ExtensionTester):
    ...
    def on_start(self, ten_env_tester: TenEnvTester) -> None:
        """Called when test starts, sends a TTS request."""
        ten_env_tester.log_info("Flush error test started, sending TTS request.")
    ...
    def on_audio_frame(self, ten_env: TenEnvTester, audio_frame):
        ...
        try:
            ...
            if len(self.received_audio_chunks) == 1 and not self.flush_sent:
                ...
                ten_env.log_info("Sending flush request after first audio chunk")
                ...
                flush_data.set_property_from_json(None, flush_input.model_dump_json())
...
def test_flush_error_handling(MockElevenLabsTTS2):
    ...
    mock_client_instance.text_to_speech_ws_streaming = mock_text_to_speech_ws_streaming
    
    # Mock the handle_flush method to raise an exception
    mock_client_instance.handle_flush.side_effect = Exception("Test flush error")
    ...
    audio_data_queue.put_nowait([b"fake_audio_data_2", True, "hello word, hello agora"])
...
class ExtensionTesterDuplicateRequestError(ExtensionTester):
    ...
    def on_start(self, ten_env_tester: TenEnvTester) -> None:
        ...
        ten_env_tester.log_info("Duplicate request error test started, sending TTS requests.")
...
def test_duplicate_request_error_handling(MockElevenLabsTTS2):
    ...
    audio_data_queue.put_nowait([b"fake_audio_data_1", True, "hello word, hello agora"])
    ...
    # Verify that error was received
    assert tester.duplicate_error_received, "Duplicate request error was not received"
    assert tester.duplicate_error_data is not None, "Duplicate request error data was not received"
