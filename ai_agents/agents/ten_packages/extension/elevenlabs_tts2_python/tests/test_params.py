...
def test_default_params(MockElevenLabsTTS2):
    ...
    assert config.api_key == "${env:ELEVENLABS_TTS_KEY}", "Default api_key should be unresolved env var"
    assert config.model_id == "eleven_multilingual_v2", "Default model_id mismatch"
    assert config.voice_id == "pNInz6obpgDQGcFmaJgB", "Default voice_id mismatch"
...
class ExtensionTesterEmptyText(ExtensionTester):
    ...
    def on_start(self, ten_env_tester: TenEnvTester) -> None:
        ...
        ten_env_tester.log_info("Minimal text test started, sending TTS request.")
