...
def test_dump_functionality(MockElevenLabsTTS2):
    ...
    tester.set_test_mode_single("elevenlabs_tts2_python", json.dumps(dump_config))
    tester.run()
    ...
    assert os.path.exists(tts_dump_file), f"TTS dump file {tts_dump_file} does not exist"
...
class ExtensionTesterTextInputEnd(ExtensionTester):
    ...
    def on_start(self, ten_env_tester: TenEnvTester) -> None:
        ...
        ten_env_tester.log_info("Text input end test started, sending first TTS request.")
    ...
    def on_data(self, ten_env: TenEnvTester, data) -> None:
        ...
        if name == "tts_audio_end":
            ...
            if self.audio_end_count == 1 and not self.second_request_sent:
                ...
                ten_env.log_info("Received second tts_audio_end, stopping test.")
                ten_env.stop_test()
...
def test_text_input_end_logic(MockElevenLabsTTS2):
    ...
    audio_data_queue.put_nowait([b"fake_audio_data_4", True, "this should be ignored"])
    ...
    assert tester.audio_end_count == 2, f"Expected 2 audio end events, got {tester.audio_end_count}"
