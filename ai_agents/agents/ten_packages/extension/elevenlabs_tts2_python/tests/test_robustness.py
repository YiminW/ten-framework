...
def test_large_text_handling(MockElevenLabsTTS2):
    ...
    audio_data_queue.put_nowait([b"fake_audio_data_2" * 1000, True, "hello word, hello agora"])
    ...
    assert len(tester.received_audio_chunks) > 0, "No audio chunks were received"
...
class ExtensionTesterConcurrentRequests(ExtensionTester):
    ...
    def on_start(self, ten_env_tester: TenEnvTester) -> None:
        ...
        ten_env_tester.log_info("Concurrent requests test started, sending multiple TTS requests.")
...
def test_concurrent_requests_handling(MockElevenLabsTTS2):
    ...
    for i in range(3):
        audio_data_queue.put_nowait([b"fake_audio_data_1", False, ""])
        audio_data_queue.put_nowait([b"fake_audio_data_2", True, f"hello word {i}"])
    ...
    assert tester.audio_end_count == 3, f"Expected 3 audio end events, got {tester.audio_end_count}"
...
class ExtensionTesterStressTest(ExtensionTester):
    ...
    def on_start(self, ten_env_tester: TenEnvTester) -> None:
        ...
        ten_env_tester.log_info("Stress test started, sending many TTS requests.")
...
def test_stress_test(MockElevenLabsTTS2):
    ...
    assert tester.audio_end_count >= 8, f"Expected at least 8 audio end events, got {tester.audio_end_count}"
    assert tester.error_count <= 2, f"Expected at most 2 errors, got {tester.error_count}"
...
class ExtensionTesterFlushRobustness(ExtensionTester):
    ...
    def on_start(self, ten_env_tester: TenEnvTester) -> None:
        """Called when test starts, sends a TTS request."""
        ten_env_tester.log_info("Flush robustness test started, sending TTS request.")
...
def test_flush_robustness(MockElevenLabsTTS2):
    ...
    audio_data_queue.put_nowait([b"fake_audio_data_3", True, "hello word, hello agora"])
